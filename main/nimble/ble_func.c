...
{
	ESP_LOGI(tag,
			 "handle=%d our_ota_addr_type=%d our_ota_addr=" MACSTR " our_id_addr_type=%d our_id_addr=" MACSTR
			 " peer_ota_addr_type=%d peer_ota_addr=" MACSTR " peer_id_addr_type=%d peer_id_addr=" MACSTR
			 " conn_itvl=%d conn_latency=%d supervision_timeout=%d "
			 "encrypted=%d authenticated=%d bonded=%d\n",
			 desc->conn_handle, desc->our_ota_addr.type, MAC2STR_REV(desc->our_ota_addr.val), desc->our_id_addr.type,
			 MAC2STR_REV(desc->our_id_addr.val), desc->peer_ota_addr.type, MAC2STR_REV(desc->peer_ota_addr.val),
			 desc->peer_id_addr.type, MAC2STR_REV(desc->peer_id_addr.val), desc->conn_itvl, desc->conn_latency,
			 desc->supervision_timeout, desc->sec_state.encrypted, desc->sec_state.authenticated,
			 desc->sec_state.bonded);
}

int user_parse(const struct ble_hs_adv_field* data, void* arg)
{
	ESP_LOGI(tag, "Parse data: field len %d, type %x, total %d bytes", data->length, data->type,
...
static void bleprph_advertise(void)
{
	...
	fields.appearance			 = HID_MOUSE_APPEARENCE;
	fields.appearance_is_present = 1;

	fields.uuids16				 = (ble_uuid16_t[]) {BLE_UUID16_INIT(GATT_UUID_HID_SERVICE)};
	...
	{
		ESP_LOGE(tag, "Too long advertising data: name %s, appearance %x, uuid16 %x, advsize = %d", name,
...
static int bleprph_gap_event(struct ble_gap_event* event, void* arg)
{
	...
	case BLE_GAP_EVENT_CONNECT:
		...
		if(event->connect.status == 0)
		{
			...
			if(rc != 0)
			{
				ESP_LOGE(tag, "Update params error: %d", rc);
			}
			hid_clean_vars(&desc);
	...
	case BLE_GAP_EVENT_SUBSCRIBE:
		ESP_LOGI(tag,
				 "subscribe event; conn_handle=%d attr_handle=%04X "
		...
		hid_set_notify(event->subscribe.attr_handle, event->subscribe.cur_notify, event->subscribe.cur_indicate);
		return 0;

	case BLE_GAP_EVENT_NOTIFY_TX:
		ESP_LOGD(tag, "notify event; status=%d conn_handle=%d attr_handle=%04X type=%s", event->notify_tx.status,
				 event->notify_tx.conn_handle, event->notify_tx.attr_handle,
				 event->notify_tx.indication ? "indicate" : "notify");
		return 0;

	case BLE_GAP_EVENT_MTU:
		ESP_LOGI(tag, "mtu update event; conn_handle=%d cid=%d mtu=%d", event->mtu.conn_handle, event->mtu.channel_id,
				 event->mtu.value);
		return 0;
...
void ble_init()
{
	...
	/* Set GAP appearance */
	rc = ble_svc_gap_device_appearance_set(HID_MOUSE_APPEARENCE); /* HID Mouse*/
	assert(rc == 0);
