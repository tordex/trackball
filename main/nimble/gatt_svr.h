...
#define BLE_SVC_DIS_PNP_INFO_DEFAULT                                                                                   \
	{0x04, 0x24, 0x56, 0x00, 0x01, 0x00, 0x01}                                                                         \
	// Vendor ID Source (1 byte), Vendor ID (2 bytes), Product ID (2 bytes), Product Version (2 bytes)
...
#define HID_CC_RPT_SET_CHANNEL(s, x)                                                                                   \
	...
	(s)[0] |= ((x) & 0x03) << 4
#define HID_CC_RPT_SET_VOLUME(s, x)                                                                                    \
...
#define HID_MOUSE_APPEARENCE		   0x03c2 // appearance field in advertising packet
	...
	int ble_svc_battery_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt* ctxt,
							   void* arg);
