...
int hid_svr_chr_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt* ctxt, void* arg)
{
	...
	ESP_LOGI("", "%s: UUID %04X attr %04X arg %d op %d", __FUNCTION__, uuid16, attr_handle, (int) arg, ctxt->op);
	...
	case GATT_UUID_HID_INFORMATION:
		...
		rc = os_mbuf_append(ctxt->om, HidInfo, HID_INFORMATION_LEN);
	...
	case GATT_UUID_EXT_RPT_REF_DESCR:
		{
			...
			rc = os_mbuf_append(ctxt->om, (uint8_t*) &HidExtReportRefDesc, sizeof(HidExtReportRefDesc));
			return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
		}
	...
	case GATT_UUID_HID_PROTO_MODE:
		{
			...
			if(ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR)
			{
				...
				rc = os_mbuf_append(ctxt->om, &HidProtocolMode, sizeof(HidProtocolMode));
				return rc == 0 ? 0 : BLE_ATT_ERR_INSUFFICIENT_RES;
			...
			} else if(ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR)
			{
				...
				rc = gatt_svr_chr_write(ctxt->om, 1, sizeof(new_protocol_mode), &new_protocol_mode, NULL);
				if(!rc)
				{
					...
					ESP_LOGI(tag, "Received new protocol mode: %d", (int) new_protocol_mode);
				}
...
int ble_svc_report_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt* ctxt, void* arg)
{
	uint16_t uuid16		= ble_uuid_u16(ctxt->chr->uuid);
	int		 handle_num = (int) arg;
	int		 rc			= BLE_ATT_ERR_UNLIKELY;

	ESP_LOGD("", "%s: UUID %04X attr %04X arg %d op %d", __FUNCTION__, uuid16, attr_handle, (int) arg, ctxt->op);
	...
	do
	{
		...
		if(uuid16 == GATT_UUID_RPT_REF_DESCR)
		{
			...
			if(rpt_ind != -1)
			{
				rc = os_mbuf_append(ctxt->om, (uint8_t*) Hid_report_ref_data[rpt_ind].hidReportRef, HID_REPORT_REF_LEN);
				if(rc)
				{
		...
		/* reports read */
		if(ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR &&
		   ((uuid16 == GATT_UUID_HID_REPORT) || (uuid16 == GATT_UUID_HID_BT_MOUSE_INPUT)))
		{
 ...
 * BAS access function
 */
int ble_svc_battery_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt* ctxt, void* arg)
{
	uint16_t uuid16 = ble_uuid_u16(ctxt->chr->uuid);
	int		 rc		= 0;
 ...
 * characteristic.
 */
int ble_svc_dis_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt* ctxt, void* arg)
{
	...
	ESP_LOGI("", "%s: UUID %04X attr %04X arg %d op %d", __FUNCTION__, uuid, attr_handle, (int) arg, ctxt->op);
...
void gatt_svr_register_cb(struct ble_gatt_register_ctxt* ctxt, void* arg)
{
	...
	case BLE_GATT_REGISTER_OP_SVC:
		ESP_LOGI("service", "uuid16 %s handle=%d (%04X)", ble_uuid_to_str(ctxt->svc.svc_def->uuid, buf),
				 ctxt->svc.handle, ctxt->svc.handle);
		break;

	case BLE_GATT_REGISTER_OP_CHR:
		ESP_LOGI("charact", "uuid16 %s arg %d def_handle=%d (%04X) val_handle=%d (%04X)",
				 ble_uuid_to_str(ctxt->chr.chr_def->uuid, buf), (int) ctxt->chr.chr_def->arg, ctxt->chr.def_handle,
				 ctxt->chr.def_handle, ctxt->chr.val_handle, ctxt->chr.val_handle);
		break;

	case BLE_GATT_REGISTER_OP_DSC:
		ESP_LOGI("descrip", "uuid16 %s arg %d handle=%d (%04X)", ble_uuid_to_str(ctxt->dsc.dsc_def->uuid, buf),
				 (int) ctxt->dsc.dsc_def->arg, ctxt->dsc.handle, ctxt->dsc.handle);
		break;
	}
}

#define BREAK_IF_NOT_ZERO(EXPR2TEST)                                                                                   \
	if((EXPR2TEST) != 0)                                                                                               \
	break
